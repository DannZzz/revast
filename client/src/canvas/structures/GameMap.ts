...
export class GameMap {
  static draw(mapDto: MapDto, config: NodeConfig = {}): Konva.Group {
    ...
    const aP = (p: Point) => new Point(p.x * mapDto.tileSize.width, p.y * mapDto.tileSize.height)
    const aS = (s: Size) => new Size(s.width * mapDto.tileSize.width, s.height * mapDto.tileSize.height)
